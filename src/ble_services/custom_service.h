...
/**
 * @file
 * @defgroup bt_custom_srv Nordic UART (CUSTOM_SRV) GATT Service
 * @{
 * @brief Nordic UART (CUSTOM_SRV) GATT Service API.
 */
...
/** @brief UUID of the CUSTOM_SRV Service. **/
#define BT_UUID_CUSTOM_SRV_VAL \
	BT_UUID_128_ENCODE(0x6e40FFFE, 0xb5a3, 0xf393, 0xe0a9, 0xe50e24dcca9e)
...
// #define BT_UUID_CUSTOM_SRV_SERVICE   BT_UUID_DECLARE_128(BT_UUID_CUSTOM_SRV_VAL)
// #define BT_UUID_CUSTOM_SRV_RX        BT_UUID_DECLARE_128(BT_UUID_CUSTOM_SRV_RX_VAL)
// #define BT_UUID_CUSTOM_SRV_TX        BT_UUID_DECLARE_128(BT_UUID_CUSTOM_SRV_TX_VAL)

#define BT_UUID_CUSTOM_SRV_SERVICE   BT_UUID_DECLARE_16(0xFFFE)
#define BT_UUID_CUSTOM_SRV_RX        BT_UUID_DECLARE_16(0X0002)
#define BT_UUID_CUSTOM_SRV_TX        BT_UUID_DECLARE_16(0X0003)

/** @brief CUSTOM_SRV send status. */
enum bt_custom_srv_send_status {
	/** Send notification enabled. */
	BT_CUSTOM_SRV_SEND_STATUS_ENABLED,
	/** Send notification disabled. */
	BT_CUSTOM_SRV_SEND_STATUS_DISABLED,
};

/** @brief Pointers to the callback functions for service events. */
struct bt_custom_srv_cb {
	/** @brief Data received callback.
	 *
	 * The data has been received as a write request on the CUSTOM_SRV RX
	 * Characteristic.
	 *
	 ...
	 *
	 * The data has been sent as a notification and written on the CUSTOM_SRV TX
	 * Characteristic.
	 *
	 ...
	 *
	 * Indicate the CCCD descriptor status of the CUSTOM_SRV TX characteristic.
	 *
	 * @param[in] status Send notification status.
	 */
	void (*send_enabled)(enum bt_custom_srv_send_status status);
...
/**@brief Get maximum data length that can be used for @ref bt_custom_srv_send.
 *
 * @param[in] conn Pointer to connection Object.
 *
 * @return Maximum data length.
 */
static inline uint32_t bt_custom_srv_get_mtu(struct bt_conn *conn)
{
